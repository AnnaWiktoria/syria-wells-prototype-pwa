    ...
    if (currentWellCoords) {
        setTimeout(() => {
            ...
            var modalMarker = L.marker(currentWellCoords, {
                icon: chooseIcon(getWellProperty(currentWellName, "waterAvailability") || { waterAvailability: "Moderate" }), // Fix default
                draggable: true,
            }).addTo(modalMap);
...
function submitReport() {
    ...
    if (currentWellName && currentWellCoords) {
        ...
        if (existingWellIndex !== -1) {
            ...
            wells.push(newWell);
            const m = L.marker(newWell.coords, { icon: chooseIcon(newWell) }).addTo(map);
