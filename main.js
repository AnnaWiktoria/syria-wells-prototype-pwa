...
// --- DATA ---
// Zmienna 'wells' musi być zdefiniowana TUTAJ, na górze, aby funkcje poniżej ją widziały.
const wells = [
	{
		name: "well_A_name",
		village: "cityAleppo",
...
// --- DOM ELEMENTS (will be re-fetched if needed, but good to have refs) ---
...
function saveWellsToStorage() {
	// FIX: Remove 'marker' property to avoid circular JSON error
...
function handleMapTapForNewWell(e) {
	...
	// Small delay to ensure modal logic doesn't conflict
	setTimeout(() => {
...
function openModal(
	...
	if (currentWellCoords) {
		setTimeout(() => {
			...
			// Pobieramy dane istniejącej studni (jeśli edytujemy) lub tworzymy atrapę dla nowej
			const existingWell = getWellProperty(currentWellName, "waterAvailability")
				? {
						status: getWellProperty(currentWellName, "status"),
						waterAvailability: getWellProperty(
							currentWellName,
							"waterAvailability"
						),
						waterQuality: getWellProperty(currentWellName, "waterQuality"),
				  }
				: { status: [], waterAvailability: "Moderate", waterQuality: [] };

			// POPRAWKA: Przekazujemy obiekt do chooseIcon
			var modalMarker = L.marker(currentWellCoords, {
				icon: chooseIcon(existingWell),
				draggable: true,
			}).addTo(modalMap);
...
function submitReport() {
	...
	const newWellData = {
		...
		status: [selectedStatusValue], // Ważne: to jest tablica
		waterAvailability,
		waterQuality,
		wellType,
		wellDepth,
	...
	if (currentWellName && currentWellCoords) {
		...
		if (existingWellIndex !== -1) {
			...
			if (updatedWell.marker) {
				// POPRAWKA: Przekazujemy CAŁY obiekt updatedWell, a nie tylko waterAvailability
				updatedWell.marker.setIcon(chooseIcon(updatedWell));
				updatedWell.marker.setPopupContent(createWellPopupContent(updatedWell));
			}
		} else {
			// Fallback (gdyby nie znaleziono indeksu)
			const newWell = { ...newWellData, marker: null };
			wells.push(newWell);
			const m = L.marker(newWell.coords, { icon: chooseIcon(newWell) }).addTo(
				map
			);
		...
		const newWell = { ...newWellData, marker: null };
		wells.push(newWell);
		// POPRAWKA: Przekazujemy CAŁY obiekt newWell
		const m = L.marker(newWell.coords, { icon: chooseIcon(newWell) }).addTo(
			map
		);
		m.bindPopup(createWellPopupContent(newWell));
...
// Also expose legend function if needed, but it's internal logic usually.
