...
// 1. Definicja funkcji handleMapTapForNewWell MUSI BYĆ PRZED openModal
    ...
    if (currentWellCoords) {
        setTimeout(() => {
            ...
            var modalMarker = L.marker(currentWellCoords, {
                icon: chooseIcon(getWellProperty(currentWellName, "waterAvailability") || "Moderate"),
                draggable: true,
            }).addTo(modalMap);
...
function submitReport() {
    ...
    if (currentWellName && currentWellCoords) {
        ...
        if (existingWellIndex !== -1) {
            ...
            wells.push(newWell);
            const m = L.marker(newWell.coords, { icon: chooseIcon(newWell.waterAvailability) }).addTo(map);
...
// 5. Expose Global Functions for HTML onClick
...
function setLanguage(lang) {
    ...
    if (typeof installBanner !== "undefined" && installBanner !== null && 
        typeof installBtn !== "undefined" && installBtn !== null) {
        updateInstallTexts();
    }
