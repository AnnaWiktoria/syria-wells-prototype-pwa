...
const translations = {
	en: {
		...
		thankYouOffline:
			"Thank you. You are currently offline. Your report has been <b>securely saved</b> on your device and will be sent automatically once you are back online.",
		offlineMessage:
		...
		rateLimitError: "Please wait 1 minute before submitting another report.",
		consentRequired: "Please agree to share this information.",
		coordinatesRequired: "Location is required.",
		locationOutsideSyria: "Location must be within Syria.",
	...
	ar: {
		...
		thankYouOffline:
			"شكراً لك. أنت غير متصل بالإنترنت. تم حفظ تقريرك <b>بأمان</b> على جهازك وسيتم إرساله تلقائياً بمجرد استعادة الاتصال.",
		offlineMessage:
		...
		rateLimitError: "يرجى الانتظار دقيقة واحدة قبل إرسال تقرير آخر.",
		consentRequired: "يرجى الموافقة على مشاركة هذه المعلومات.",
		coordinatesRequired: "الموقع مطلوب.",
		locationOutsideSyria: "يجب أن يكون الموقع داخل سوريا.",
		submitError: "فشل إرسال التقرير. يرجى المحاولة مرة أخرى.",
		clearDataBtn: "مسح جميع البيانات",
		dataClearConfirm:
			"هل أنت متأكد من حذف جميع البيانات المحفوظة؟ لا يمكن التراجع عن ذلك.",
	...
	ku: {
		...
		thankYouOffline:
			"Spas. Hûn niha oflîn in. Rapora we <b>bi ewlehî</b> hate tomarkirin û dema ku hûn dîsa serhêl bibin dê bixweber were şandin.",
		offlineMessage:
		...
		rateLimitError:
			"Ji kerema xwe berî ragihandina rapora din deqîqeyek li bendê bimîne.",
		consentRequired: "Ji kerema xwe razî bibin ku ev agahdarî parve bikin.",
		coordinatesRequired: "Cîh pêwîst e.",
		locationOutsideSyria: "Cîh divê li hundurê Sûriyê be.",
		submitError: "Rapora nehat şandin. Ji kerema xwe dîsa biceribîne.",
		clearDataBtn: "Hemî Daneyan Paqij Bike",
		dataClearConfirm:
			"Ma hûn bawer in ku dixwazin hemî daneyên tomarkirî jê bibin? Ev nayê vegerandin.",
		dataCleared: "Hemî dane hate paqij kirin.",
	},
};
/* ==========================================================================
   SECURITY UTILITIES
...
const SecurityUtils = {
	...
	/**
	 * Validate coordinates are within Syria
	 * Prevents fake reports from outside the country
	 */
	isInSyria(lat, lng) {
		// Syria approximate bounds: 32-37.5°N, 35-42.5°E
		return lat >= 32 && lat <= 37.5 && lng >= 35 && lng <= 42.5;
	},

	/**
	 * Simple rate limiting (client-side)
	 * Prevents spam by limiting reports to 1 per minute
	 */
	canSubmitReport() {
		const lastSubmit = localStorage.getItem("lastReportTime");
		if (!lastSubmit) return true;

		const timeSinceLastReport = Date.now() - parseInt(lastSubmit);
		const MIN_INTERVAL = 60 * 1000; // 1 minute

		return timeSinceLastReport > MIN_INTERVAL;
	},
...
/**
 * Save wells to localStorage with error handling
 * Handles quota exceeded and data corruption
 */
function saveWellsToStorage() {
	try {
		// Remove marker objects (can't be serialized)
		...
		// ============================================
		// SECURITY: Check size limit (localStorage ~5-10MB)
		// ============================================
		if (serialized.length > 5 * 1024 * 1024) {
			// 5MB
			console.warn("Data too large for localStorage");

			// Keep only recent 100 wells
			const recent = dataToSave
				.sort((a, b) => b.lastUpdate - a.lastUpdate)
				.slice(0, 100);

			localStorage.setItem("wellsData", JSON.stringify(recent));

			// Notify user
			alert("Storage limit reached. Keeping only 100 most recent wells.");
		} else {
			localStorage.setItem("wellsData", serialized);
		}
	} catch (error) {
		console.error("Failed to save wells:", error);
		...
		if (error.name === "QuotaExceededError") {
			try {
				// Try saving only last 50 wells
				const recent = wells.slice(-50).map(w => {
					const { marker, ...rest } = w;
					return rest;
				});

				localStorage.setItem("wellsData", JSON.stringify(recent));
				alert("Storage full. Saved only 50 most recent wells.");
			} catch (e) {
				console.error("Could not save even reduced data:", e);
				alert("Critical: Cannot save data. Storage full.");
			}
		}
	}
}
...
function loadWellsFromStorage() {
	try {
		...
		if (!Array.isArray(parsed)) {
			console.error("Invalid wells data: not an array");
			localStorage.removeItem("wellsData"); // Clear corrupted data
			return;
		}

		// ============================================
		// SECURITY: Validate each well
		// ============================================
		const validated = parsed.filter(w => {
			// Check required fields exist
			if (!w || !w.coords || !Array.isArray(w.coords)) {
				return false;
			}

			// Check coords are numbers
			if (typeof w.coords[0] !== "number" || typeof w.coords[1] !== "number") {
				return false;
			}

			// Check coords are in Syria
			if (!SecurityUtils.isInSyria(w.coords[0], w.coords[1])) {
				console.warn("Skipping well outside Syria:", w);
				return false;
			}
		...
		// LOAD VALIDATED DATA
		// ============================================
		validated.forEach(w => (w.marker = null));
		wells.length = 0;
		wells.push(...validated);

		console.log(`Loaded ${validated.length} wells from storage`);
	} catch (error) {
		...
		if (error instanceof SyntaxError) {
			...
			localStorage.removeItem("wellsData");
			alert("Corrupted data detected. Storage has been cleared.");
		}
	}
}
...
function submitReport() {
	...
	// ============================================
	if (!SecurityUtils.canSubmitReport()) {
		alert(
			t.rateLimitError ||
				"Please wait 1 minute before submitting another report."
		);
		return;
	}

	// ============================================
	// SECURITY CHECK 2: Consent Validation
	// ============================================
	const consentGiven = document.getElementById("consentCheckbox")?.checked;
	if (!consentGiven) {
		alert(t.consentRequired || "Please agree to share this information.");
		return;
	}

	// ============================================
	// SECURITY CHECK 3: Coordinates Validation
	// ============================================
	if (!currentWellCoords) {
		alert(t.coordinatesRequired || "Location is required.");
		return;
	}

	const [lat, lng] = currentWellCoords;
	if (!SecurityUtils.isInSyria(lat, lng)) {
		alert(t.locationOutsideSyria || "Location must be within Syria.");
		return;
	}
	...
	// Get dropdown values (safe - predefined options)
	const selectedStatusValue =
		document.getElementById("conditionSelect")?.value || "Functional";
	const waterAvailability =
		document.getElementById("availabilitySelect")?.value || "Moderate";
	const wellType = document.getElementById("typeSelect")?.value || "Other";
	const wellDepth = document.getElementById("depthSelect")?.value || "";

	// Get checkboxes (safe - predefined values)
	...
	// ⚠️ CRITICAL: Sanitize user-generated content
	const additionalNotes = SecurityUtils.sanitizeHTML(
	...
	const newWellData = {
		...
		name: currentWellName || `Well_${Date.now()}`,
		coords: [lat, lng],
		...
		wellType,
		wellDepth,
		additionalNotes, // ✅ Already sanitized above
		consentGiven: true,
		lastUpdate: Date.now(),
	...
	// ============================================
	// UPDATE OR CREATE WELL (with error handling)
	// ============================================
	try {
		if (currentWellName && currentWellCoords) {
			// Updating existing well
			...
			if (existingWellIndex !== -1) {
				...
				if (updatedWell.marker) {
					...
					updatedWell.marker.setPopupContent(
						createWellPopupContent(updatedWell)
					);
				}
			} else {
				// Well not found, create new
				addNewWell(newWellData);
			}
		...
		// ============================================
		saveWellsToStorage();
		SecurityUtils.recordSubmission(); // ✅ Record for rate limiting
		map.closePopup();

		// ============================================
		// SUCCESS MESSAGE
		// ============================================
		const finalMessage = isOfflineNow ? t.thankYouOffline : t.thankYouOnline;
		showThankYouMessage(finalMessage);
	} catch (error) {
...
function addNewWell(wellData) {
	const newWell = { ...wellData, marker: null };
	wells.push(newWell);

	const color = chooseColorName(newWell);
	const iconObj = getPinIcon(color, newWell.isOffline);
	const marker = L.marker(newWell.coords, { icon: iconObj }).addTo(map);
	marker.bindPopup(createWellPopupContent(newWell));
	newWell.marker = marker;
}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	// ============================================
	// PRIVACY FEATURE: Clear All Data Button
	// ============================================

	/**
	 * Adds button to clear all locally stored data
	 * Important for shared devices in field offices
	 */
	function addClearDataButton() {
		...
		const updateButtonText = () => {
			const t = translations[currentLanguage];
			clearBtn.innerHTML = "🗑️ " + (t.clearDataBtn || "Clear All Data");
		};
		updateButtonText();
		...
		if (wells.length > 0 || localStorage.getItem("wellsData")) {
			clearBtn.style.display = "block";
		}

		// Add to container
		floatingContainer.appendChild(clearBtn);

		// Update text when language changes
		document
			.getElementById("langEn")
			?.addEventListener("click", updateButtonText);
		document
			.getElementById("langAr")
			?.addEventListener("click", updateButtonText);
		document
